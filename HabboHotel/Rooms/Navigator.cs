...
namespace Butterfly.HabboHotel.Navigators
{
    class Navigator
    {
        ...
        {
            // Caricamento delle categorie private
            dbClient.setQuery("SELECT id, caption, min_rank FROM navigator_flatcats WHERE CAST(enabled AS CHAR) = '1'");
            DataTable dPrivCats = dbClient.getTable();

            // Caricamento delle stanze pubbliche
            dbClient.setQuery("SELECT * FROM navigator_publics WHERE CAST(enabled AS CHAR) = '1' ORDER BY ordernum ASC");
            DataTable dPubItems = dbClient.getTable();

            // Debug per verificare se le stanze vengono caricate correttamente
            Console.WriteLine("Categorie private caricate: " + (dPrivCats != null ? dPrivCats.Rows.Count : 0));
            Console.WriteLine("Stanze pubbliche caricate: " + (dPubItems != null ? dPubItems.Rows.Count : 0));
            ...
            if (dPrivCats != null)
            {
                foreach (DataRow Row in dPrivCats.Rows)
                {
                    ...
                    PrivateCategories.Add(id, new FlatCat(id, caption, minRank));
                }
            }
            ...
            if (dPubItems != null)
            {
                ...
                {
                    int id = Convert.ToInt32(Row["id"]);
                    int bannerType = Convert.ToInt32(Row["bannertype"]);
                    string caption = Row["caption"].ToString();
                    string image = Row["image"].ToString();
                    PublicImageType imageType = Row["image_type"].ToString().ToLower() == "internal" ? PublicImageType.INTERNAL : PublicImageType.EXTERNAL;
                    uint roomId = Convert.ToUInt32(Row["room_id"]);
                    int categoryParentId = Convert.ToInt32(Row["category_parent_id"]);
                    bool isCategory = ButterflyEnvironment.EnumToBool(Row["category"].ToString());
                    bool isRecommended = ButterflyEnvironment.EnumToBool(Row["recommended"].ToString());

                    PublicItems.Add(id, new PublicItem(id, bannerType, caption, image, imageType, roomId, categoryParentId, isCategory, isRecommended));
                }
            }
        }
        ...
        internal ServerMessage SerializeEventListing(int Category)
        {
            ...
            List<Room> EventRooms = ButterflyEnvironment.GetGame().GetRoomManager().GetEventRoomsForCategory(Category);
