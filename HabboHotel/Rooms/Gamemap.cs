...
namespace Butterfly.HabboHotel.Rooms
{
    class Gamemap
    {
        ...
        internal void TeleportToItem(RoomUser user, RoomItem item)
        {
            ...
            RoomItem seat = null;
            List<RoomItem> items = room.GetRoomItemHandler().GetFurniObjects(item.GetX, item.GetY);
            foreach (RoomItem mItem in items)
            {
                if (mItem.GetBaseItem().IsSeat)
                {
                    seat = mItem;
                }
            }
            if (seat != null)
            {
                ...
                user.Z = mDynamicModel.SqFloorHeight[user.X, user.Y];
                if (user.isFlying)
                    user.Z += 4 + (0.5 * Math.Sin(0.7 * user.flyk));
                user.RotHead = seat.Rot;
                user.RotBody = seat.Rot;
